...
try {
  ...
  const renderPngBase64 = async (size) => {
  ...
  replace(/(<link rel="icon"[^>]*sizes="192x192"[^>]*href="data:image\/png;base64,)[^"]+(")/, png192);
  // <link rel="apple-touch-icon" sizes="192x192" href="data:image/png;base64,...">
  replace(/(<link rel="apple-touch-icon"[^>]*sizes="192x192"[^>]*href="data:image\/png;base64,)[^"]+(")/, png192);
  // <link rel="apple-touch-icon" sizes="512x512" href="data:image/png;base64,...">
  replace(/(<link rel="apple-touch-icon"[^>]*sizes="512x512"[^>]*href="data:image\/png;base64,)[^"]+(")/, png512);
  // manifest icons array: {"src":"data:image/png;base64,...","sizes":"192x192",...}
  replace(/("src":"data:image\/png;base64,)[^"]+("(?:[^{}]*)"sizes":"192x192")/, png192);
  // manifest icons array: BOTH "any" and "maskable" 512x512 entries reuse the same 512 PNG
  replace(/("src":"data:image\/png;base64,)[^"]+("(?:[^{}]*)"sizes":"512x512")/g, png512);

  fs.writeFileSync('index.html', html);
  console.log(`Icons updated: ${replacements} references (192px ${png192.length}b64 chars, 512px ${png512.length}b64 chars)`);
} catch (e) {
  console.error('inline-icons.mjs failed:', e && e.stack ? e.stack : e);
  process.exit(1);
}
